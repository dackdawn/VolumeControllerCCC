...
{
    public partial class VolumeBoxHorizon : Form
    {
        ...
        public VolumeBoxHorizon(Form parent)
        {
            _parent = parent;
            InitializeComponent();
            Init();
        ...
        {
            if (offset) {
                SetBounds(X, Y - Bounds.Height - Offset, Bounds.Width, Bounds.Height);
            }
            else {
                SetBounds(X, Y + Offset, Bounds.Width, Bounds.Height);
            }
            DrawVolumeBoxCur();
            Show();
            hideTimer.Stop();
        ...
        private void DrawVolumeBoxCur()
        {
            ...
            int width = (int)(widthMax * (curVolume * 1.0f / 100.0f));
            ...
            Rectangle rectangleCur = new Rectangle(0, 0, width - 1, height - 1);
            // 先绘画音量条背景
            FillRoundRectangle(g, rectangleBg, Color.FromArgb(240, 248, 255), (height - 1) / 2);
            // 然后再绘制当前音量
            FillRoundRectangle(g, rectangleCur, Color.FromArgb(64, 158, 255), (height - 1) / 2);
            // 最后绘制边框
            g.DrawPath(new Pen(Color.FromArgb(242, 246, 252)), GetRoundRectangle(rectangleBg, (height - 1) / 2));
            g.Dispose();
